...
class JarvisUI:
    def __init__(self, face_path, size=None):
        self.root = tk.Tk()
        self.root.title("B.I.N.L.A.P — MARK XXX")
    ...
    def _animate(self):
        ...
        is_active = self.speaking or self.audio_volume > 0.02
        if self.audio_volume > 0.02:
            self.has_spoken_once = True
            self.target_scale = 1.0 + (self.audio_volume * 0.3)
            self.target_halo  = 80 + (self.audio_volume * 200)

        # Apply smooth volume decay
        if self.audio_volume > 0:
            self.audio_volume = max(0.0, self.audio_volume - 0.03)

        sp = 0.6 if self.audio_volume > 0.02 else (0.35 if self.speaking else 0.16)
        self.scale  += (self.target_scale - self.scale) * sp
        self.halo_a += (self.target_halo  - self.halo_a) * sp

        for i, spd in enumerate([1.2, -0.8, 1.9] if is_active else [0.5, -0.3, 0.82]):
            self.rings_spin[i] = (self.rings_spin[i] + spd) % 360

        self.scan_angle  = (self.scan_angle  + (2.8 if is_active else 1.2)) % 360
        self.scan2_angle = (self.scan2_angle + (-1.7 if is_active else -0.68)) % 360

        pspd  = 3.8 if is_active else 1.8
        limit = self.FACE_SZ * 0.72
        new_p = [r + pspd for r in self.pulse_r if r + pspd < limit]
        if len(new_p) < 3 and random.random() < (0.06 if is_active else 0.022):
            new_p.append(0.0)
        self.pulse_r = new_p
    ...
    def _draw(self):
        ...
        if self.speaking or self.audio_volume > 0.02:
            self.has_spoken_once = True
            
        if self.has_spoken_once:
        ...
        for i in range(N):
            if self.audio_volume > 0.02:
                hb = int(BH * self.audio_volume * random.uniform(0.6, 1.5))
                hb = min(BH, max(3, hb))
    ...
    def _show_setup_ui(self):
        ...
        tk.Label(self.setup_frame,
                 text="Enter your Gemini API key to boot B.I.N.L.A.P.",
    ...
    def _save_api_keys(self):
        ...
        self.status_text = "ONLINE"
        self.write_log("SYS: Systems initialised. B.I.N.L.A.P online.")